...
If $M \, dx + N \, dy = 0$ is a homogeneous DE, then the change of variable $y = vx$ transforms the DE into a separable equation in $v$ and $x$.
...
If all three are satisfied, the problem is well-posed.

If any one condition is not satisfied, the problem is ill-posed.

---

## Lemma

If $f: \mathbb{R}^n \to \mathbb{R}$ is a linear function, then

$$
F(x) = a_1x_1 + \ldots + a_nx_n
$$

where $a_1, \ldots, a_n$ are constants and $x = (x_1, x_2, \ldots, x_n) \in \mathbb{R}^n$.
...
If $F(x, y, y', y'', \ldots, y^{(n)}) = 0$, $x \in I$ is a homogeneous linear equation of order $n$ defined on $I$, then it can be written as

$$
F(x, y, \ldots, y^{(n)}) = a_0(x) \, y^{(n)} + a_1(x) \, y^{(n-1)} + \ldots + a_n(x) \, y = 0
$$

for each $x \in I$, where $a_0, a_1, \ldots, a_n$ are functions defined on $I$.
