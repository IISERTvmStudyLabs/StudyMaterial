...
$$
= (Py_1^2 + qy_1 + r) + (2Py_1 + q)z^{-1} + Pz^{-2}
$$

$$
= y_1' + (2Py_1 + q)z^{-1} + Pz^{-2}
$$

$$
\Rightarrow z' + (2Py_1 + q)z + P = 0
$$
...
$$
\Rightarrow y' + ay = 0, \text{ then } y = ce^{-ax} \text{ is a solution.}
$$
...
$$
L(ce^{rx}) = (r^2 + a_1r + a_2) \cdot e^{rx} = P(r) \cdot e^{rx}
$$
...
Then $e^{r_1x}$ and $e^{r_2x}$ are solutions of $L(y) = 0$.
...
If $r_1, r_2$ are distinct roots of $P(r)$, then the functions:
...
If $r$ is a repeated root of $P(r)$, then
